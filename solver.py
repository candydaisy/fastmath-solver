"""
FastMath solver core.

Given a list of input numbers and a target, brute-forces combinations of
+, -, *, /, **, sqrt, and factorial (plus a sigma/summation reduction for
3-number groups) to find an expression that lands on or near the target.

This is the same logic as the original fastmath_solver.py CLI tool, with
the input loop stripped out so it can be called from a web request.
"""
