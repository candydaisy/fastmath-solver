...
def ocr_extract_numbers_from_image(img):
    gray = img.convert("L")
    gray = gray.resize((max(200, gray.width * 2), max(100, gray.height * 2)))
    # simple binarize
    gray = gray.point(lambda x: 0 if x < 160 else 255, '1')

    # restrict tesseract to digits to reduce false positives
    text = pytesseract.image_to_string(gray, config="--oem 3 --psm 6 outputbase digits")
    nums = re.findall(r"-?\d+", text)
    return [int(x) for x in nums]

# ---------------- GUI: region selector (fixed + transparent) ----------------

class RegionSelector(tk.Toplevel):
    """
    Window to select a rectangle region safely.
    Works across platforms — tries to use transparent background so OCR sees underlying pixels.
    """
    def __init__(self, master):
        ...
        # Try to make the background transparent so underlying screen is visible:
        # On Windows, -transparentcolor works when the window background is set to that color.
        # If unsupported, fall back to semi-transparent alpha overlay (less ideal for OCR).
        use_transparentcolor = False
        try:
            self.attributes("-transparentcolor", "black")
            use_transparentcolor = True
        except Exception:
            try:
        ...
        # Canvas background should be black if transparentcolor is used.
        bg_color = "black" if use_transparentcolor else "#000000"
        self.canvas = tk.Canvas(self, cursor="cross", bg=bg_color, highlightthickness=0)
        ...
        # Try to keep on top while selecting
        try:
    ...
    def _screen_to_canvas(self, x_root, y_root):
        return x_root - self.winfo_rootx(), y_root - self.winfo_rooty()
...
class SolverApp(tk.Tk):
    ...
    def select_numbers_region(self):
        ...
        try:
            ...
            nums = []
            self.ocr_text.delete("1.0", "end")
            self.ocr_text.insert("1.0", f"OCR error: {e}")
    ...
    def select_target_region(self):
        ...
        if nums:
            # choose last (or you can choose largest with max(nums, key=abs))
            self.target_var.set(str(nums[-1]))
        else:
            self.target_var.set("")
